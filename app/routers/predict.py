...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Recalcule les features engineerées exactement comme dans le notebook.
    """
    ...
    # indicateur_surcharge = heures sup ET mauvais équilibre pro/perso
...
def predict_attrition(employee: EmployeeInput):
    ...
    # Reconstruction avec les noms exacts du modèle entraîné
    data = employee.model_dump(by_alias=True)
    input_df = pd.DataFrame([data])

    # Ajout des features engineerées
